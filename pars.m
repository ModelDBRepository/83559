%%%% BASIC PARAMETERS SCRIPT - Mark Humphries 12/1/2006

%% tonic firing rate calibration
...
do_urethane = 0;            % turn urethane anaesthesia emulation on/off 

%%%% dopamine
dop1 = 0.3; % tonic dopamine level (in [0 1] range) (0.3)
dop2 = 0.3; % same weight calculation as in systems level model
...
GPe = 3*neurons_per_nucleus+1:4*neurons_per_nucleus;    %GPe
GPi = 4*neurons_per_nucleus+1:5*neurons_per_nucleus;    %GPi
...
trace_n = GPe(75); % index of neuron to record in detail membrane potential, epsp, J,...
                   % (number has to be less than No neurons per nucleus)
...
mean_tau_AMPA = 0.002;   % AMPA component - 2ms excitatory current 
mean_tau_NMDA = 0.100;   % NMDA component - 100ms excitatory current 
mean_tau_GABAa = 0.003;   % GABAa - 3ms inhibitory current
...
SD22GPe_p = [0.33 0.34 0.33]; % [1 0 0];    % distal only 
GPe2STN_p = [0.3 0.4 0.3]; % [0.3 0.4 0.3];
GPe2GPi_p = [0 0.5 0.5];
GPe2GPe_p = [0 0.5 0.5];  % [0.3 0.4 0.3];
GPi2GPi_p = [0 0.5 0.5];  % [0.3 0.4 0.3];
...
if do_urethane
    ...
    spon(STN) = 11e-10; %  9e-10 with GP collaterals @ 1 
end

spon(GPe) = 3.8e-10; %2.4 e-10 with GP collaterals @ 1        
%spon(GPe) = randn(length(GPe),1)*(2e-11) + (4e-10);
%spon(GPi) = randn(length(GPi),1)*(2.5e-11) + (2.5e-10);          
spon(GPi) = 3.9e-10; %1.9e-10 with GP and GPi collaterals @ 1 
