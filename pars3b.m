...
%% selection and switching, high DA
...
%%%% simulation time
time_seconds = 5;          %total simulation time in seconds (usually 10)  

do_urethane = 0;

%%%% dopamine
dop1 = 0.8; % tonic dopamine level (in [0 1] range) (0.3)
dop2 = 0.8; % same weight calculation as in systems level model
...
if do_urethane
    ...
    spon(STN) = 11e-10; % 11e-10 with GP collaterals @ 1 
end

spon(GPe) = 3.8e-10; %3.8e-10 with GP collaterals @ 1        
%spon(GPe) = randn(length(GPe),1)*(2e-11) + (4e-10);
%spon(GPi) = randn(length(GPi),1)*(2.5e-11) + (2.5e-10);          
spon(GPi) = 3.9e-10; %3.9e-10 with GP and GPi collaterals @ 1 
