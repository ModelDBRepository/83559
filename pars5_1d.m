...
%%% Condition#2 of Magill et al. (2001) No Ctx, no DA - no STN + GP DA
...
%%%% dopamine
dop1 = 0; % tonic dopamine level (in [0 1] range) (0.3)
...
stnda = [0 0 0];  % STN coefficients for proportion of dopamine that affects [AMPA GABAa Spon] 
                         % currents (all < 1) (don't have to sum to 1)
                         % maybe spon = 0?
                         % nominally [0.25 0.5 0.1];
gpeda = [0 0];         % GPe coefficients for proportion of dopamine that affects [SD2 STN] input - NOT specific currents as above 
