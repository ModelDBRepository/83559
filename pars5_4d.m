%%%% BASIC PARAMETERS SCRIPT - Mark Humphries 9/1/2006

%%% Condition#4 of Magill et al. (2001) No Ctx, No DA 
...
GPe_GPew = 0; % -1 collaterals
GPi_GPiw = 0; % -1 collaterals

EXT_w = 0; % ctx input 1.0
...
if do_urethane
    ...
    spon(STN) = 9e-10; % 10.0e-10;       % 11e-10 with GP collaterals @ 1 
end

spon(GPe) = 3e-10; %2.7e-10;    %4.6e-10 with GP collaterals @ 1        
%spon(GPe) = randn(length(GPe),1)*(2e-11) + (4e-10);
%spon(GPi) = randn(length(GPi),1)*(2.5e-11) + (2.5e-10);          
spon(GPi) = 3.4e-10; %2.8e-10;    %3.2e-10 with GP and GPi collaterals @ 1 
